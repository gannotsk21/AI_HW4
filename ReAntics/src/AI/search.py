...
class AIPlayer(Player):
    ...
    def getMove(self, currentState):
        ...
        bestNode = root
        for i in range(3):
            frontier.sort(key=lambda node: node.depth)
            #frontier = frontier[:10]
            bestNode = self.bestMove(frontier, currentState)
            frontier.remove(bestNode)
            expanded.append(bestNode)
            frontier += self.expandNode(bestNode)
    ...
    def bestMove(self, listOfNodes, state):
    ...
    def expandNode(self, node):
        ...
        for move in moves:
            ...
            if(node.state.whoseTurn == 1):
                eval = self.heuristicStepsToGoal(state)
            else:
                eval = 0 - self.heuristicStepsToGoal(state)
